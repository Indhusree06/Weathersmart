...
export function BodyOptionsPanel({
  ...
  return (
    <div className="w-[260px] bg-card border-r border-border p-4 overflow-y-auto">
      {/* Weather Section */}
      <Card className="bg-muted border-border p-4 mb-4">
        ...
        <p className="text-sm text-foreground/80">{weather}</p>
      </Card>

      {/* Selected Items Section */}
      <Card className="bg-muted border-border p-4 mb-4">
        <h3 className="text-sm font-semibold text-foreground mb-3">Selected Items</h3>
        {outfitItems.length === 0 ? (
          <p className="text-sm text-muted-foreground italic">No items selected</p>
        ) : (
          <ul className="space-y-2">
            {outfitItems.map((item) => (
              <li key={item.id} className="text-sm text-foreground/80 flex items-start">
      ...
      <Card className="bg-muted border-border p-4">
        <h3 className="text-sm font-semibold text-foreground mb-3">Body Options</h3>
        
        {/* Mannequin Type */}
        <div className="mb-4">
          <label className="text-xs text-muted-foreground mb-2 block">
            Mannequin: <span className="text-foreground font-medium">{getMannequinLabel()}</span>
            ...
            {isAdult && !mannequinType.includes('baby') && (
              <>
                <Button
                  ...
                  className={isMale 
                    ? "bg-primary hover:bg-primary/90 text-foreground flex-1" 
                    : "border-border text-foreground/80 hover:bg-muted/80 flex-1"
                  }
                >
                  Male
                </Button>
                <Button
                  ...
                  className={!isMale 
                    ? "bg-pink-600 hover:bg-pink-700 text-foreground flex-1" 
                    : "border-border text-foreground/80 hover:bg-muted/80 flex-1"
                  }
                >
                  Female
        ...
        {/* Skin Tone */}
        <div className="mb-4">
          <label className="text-xs text-muted-foreground mb-2 block">Skin Tone</label>
        ...
        {/* Body Type */}
        <div>
          <label className="text-xs text-muted-foreground mb-2 block">Silhouette</label>
              ...
              <>
                <Button
                  ...
                  className={bodyType === 'slim'
                    ? "bg-cyan-600 hover:bg-cyan-700 text-foreground flex-1 text-xs"
                    : "border-border text-foreground/80 hover:bg-muted/80 flex-1 text-xs"
                  }
                >
                  Slim
                </Button>
